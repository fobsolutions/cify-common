...
public class CapabilityParser implements ICapabilityParser {
     ...
     */
    @Override
    public List<CapabilityConfig> parse(String capabilitiesConfigJson, String farmUrl) throws CifyCommonException {
        if (capabilitiesConfigJson == null) {
            throw new CifyCommonException("CapabilityConfig json is null. Nothing to parse.");
        }

        String strategy = getString(new JSONObject(capabilitiesConfigJson), PARAM_STRATEGY, STRATEGY_VARIATIONS);
        return parse(capabilitiesConfigJson, farmUrl, strategy);
    }

    /**
     * Parses Cify configuration file content with explicitly defined strategy
     * @return A list of CapabilityConfig objects
     */
    @Override
    public List<CapabilityConfig> parse(String capabilitiesConfigJson, String farmUrl, String strategy) throws CifyCommonException {
        JSONObject capabilities;
        try {
            capabilities = new JSONObject(capabilitiesConfigJson).getJSONObject(PARAM_CAPABILITIES);
